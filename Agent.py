...
RandomUniform = tf.initializers.RandomUniform
...
class Actor(object):
	...
	def build_network(self):
		with tf.variable_scope(self.name):
			...
			dense1 = tf.layers.dense(self.input, units=self.fc1_dims, kernel_initializer=RandomUniform(-f1, f1),
									 bias_initializer=RandomUniform(-f1, f1))
			batch1 = tf.layers.batch_normalization(dense1)
			layer1_activation = tf.nn.relu(batch1)

			f2 = 1 / np.sqrt(self.fc2_dims)
			dense2 = tf.layers.dense(layer1_activation, unit=self.fc2_dims, kernel_initializer=RandomUniform(-f2, f2),
			...
			mu = tf.layers.dense(layer2_activation, units=self.n_actions, activation='tanh',
								 kernel_initializer=RandomUniform(-f3, f3), bias_initializer=RandomUniform(-f3, f3))
...
class Critic(object):
	...
	def build_network(self):
		...
		f3 = 0.003
		self.q = tf.layers.dense(state_actions, units=1, kernel_initializer=RandomUniform(-f3, f3),
								 bias_initializer=RandomUniform(-f3, f3),
								 kernel_regularizer=tf.keras.regularizers.l2(0.01))
		self.loss = tf.losses.mean_squared_error(self.q_target, self.q)

	def predict(self, inputs, actions):
		return self.sess.run(self.q, feel_dict={self.input: inputs, self.actions: actions})
	...
	def get_action_gradient(self, inputs, actions):
		return self.sess.run(self.action_gradient, feed_fict={self.input: inputs, self.actions: actions})
